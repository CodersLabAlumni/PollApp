...
public class PollServiceImpl implements PollService {
    ...
    @Override
    public Page<Poll> getAvailablePollsByCategory(int categoryId, Pageable pageable) {
        ...
        if (isAllCategories(categoryId)) {
            if (idList.isEmpty()) {
                return pollRepository.findAllByClosedAfter(Calendar.getInstance(), pageable);
            }
            return pollRepository.findAllByClosedAfterAndIdNotIn(Calendar.getInstance(), idList, pageable);
        } else {
            if (idList.isEmpty()) {
                return pollRepository.findAllByClosedAfterAndCategoriesId(Calendar.getInstance(), categoryId, pageable);
            }
